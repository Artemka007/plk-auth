...
bool DataExportImportService::export_data(const std::string& file_path, const std::shared_ptr<const models::User>& actor) {
    try {
...
bool DataExportImportService::import_data(const std::string& file_path, const std::shared_ptr<const models::User>& actor) {
    try {
...
bool DataExportImportService::create_backup(const std::string& backup_path, const std::shared_ptr<const models::User>& actor) {
    try {
        io_handler_->println("Creating backup: " + backup_path);
        log_service_->info(models::ActionType::SYSTEM_BACKUP_CREATED,
                          "Starting backup creation: " + backup_path,
                          actor, nullptr);
...
bool DataExportImportService::restore_backup(const std::string& backup_path, const std::shared_ptr<const models::User>& actor) {
    try {
        ...
        log_service_->error(models::ActionType::SYSTEM_BACKUP_RESTORED,
                           "Backup restoration error: " + std::string(e.what()),
                           actor, nullptr);
        return false;
    }
}

bool DataExportImportService::export_logs_csv(const std::string& file_path, const std::shared_ptr<const models::User>& actor) {
    try {
        io_handler_->println("Exporting logs to CSV: " + file_path);
        log_service_->info(models::ActionType::SYSTEM_EXPORT,
                          "Starting logs export to CSV: " + file_path,
                          actor, nullptr);
        ...
        if (result) {
            ...
            log_service_->info(models::ActionType::SYSTEM_EXPORT,
                              "Logs export completed successfully: " + file_path,
                              actor, nullptr);
        } else {
            io_handler_->error("❌ Logs export failed");
            log_service_->error(models::ActionType::SYSTEM_EXPORT,
                               "Logs export failed: " + file_path,
                               actor, nullptr);
        }
        return result;
    } catch (const std::exception& e) {
...
bool DataExportImportService::export_users_csv(const std::string& file_path, const std::shared_ptr<const models::User>& actor) {
    try {
        io_handler_->println("Exporting users to CSV: " + file_path);
        log_service_->info(models::ActionType::SYSTEM_EXPORT,
                          "Starting users export to CSV: " + file_path,
                          actor, nullptr);
        ...
        if (result) {
            ...
            log_service_->info(models::ActionType::SYSTEM_EXPORT,
                              "Users export completed successfully: " + file_path,
                              actor, nullptr);
        } else {
            io_handler_->error("❌ Users export failed");
            log_service_->error(models::ActionType::SYSTEM_EXPORT,
                               "Users export failed: " + file_path,
                               actor, nullptr);
        }
        return result;
    } catch (const std::exception& e) {
...
bool DataExportImportService::export_roles_csv(const std::string& file_path, const std::shared_ptr<const models::User>& actor) {
    try {
        io_handler_->println("Exporting roles to CSV: " + file_path);
        log_service_->info(models::ActionType::SYSTEM_EXPORT,
                          "Starting roles export to CSV: " + file_path,
                          actor, nullptr);
        ...
        if (result) {
            ...
            log_service_->info(models::ActionType::SYSTEM_EXPORT,
                              "Roles export completed successfully: " + file_path,
                              actor, nullptr);
        } else {
            io_handler_->error("❌ Roles export failed");
            log_service_->error(models::ActionType::SYSTEM_EXPORT,
                               "Roles export failed: " + file_path,
                               actor, nullptr);
        }
        return result;
    } catch (const std::exception& e) {
...
void DataExportImportService::show_statistics(const std::shared_ptr<const models::User>& actor) {
    try {
        ...
        log_service_->debug(models::ActionType::PROFILE_VIEWED,
                           "Viewed database statistics - Users: " + std::to_string(user_count) + 
        ...
        log_service_->error(models::ActionType::SECURITY_VIOLATION,
                           "Error getting database statistics: " + std::string(e.what()),
                           actor, nullptr);
    }
}
